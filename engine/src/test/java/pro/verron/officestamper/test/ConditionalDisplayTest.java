...
class ConditionalDisplayTest {
    ...
    @DisplayName("Display Paragraph If Generated By Google Docs + Inline processors Integration test")
    @Test
    void conditionalDisplayOfParagraphsTest_createdByGoogleDocs() {
        var template = getResource(Path.of("ConditionalDisplayOfParagraphsGoogleDocs.docx"));
        var expected = """
                == ❬Conditional paragraph test with document generated by Google Docs❘{rtl=false}❭
                <spacing={after=0,line=276,lineRule=auto}>
                
                [<0|]❬This block is shown❘{rtl=false}❭
